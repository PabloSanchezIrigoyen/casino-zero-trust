...
function CardView({ card, hidden }: { card?: Card; hidden?: boolean }) {
  if (!card) return <div className="h-28 w-20 rounded-xl border border-white/10 bg-black/20" />;
  if (hidden) {
    return (
      <div className="grid h-28 w-20 place-items-center rounded-xl border border-[var(--gold)]/40 bg-[linear-gradient(135deg,#3b2a12,#1a140c)] font-serif text-[var(--gold)]">
        ZT
      </div>
    );
  }
  return (
    <div className="flex h-28 w-20 flex-col justify-between rounded-xl border border-white/15 bg-[#f7f1e4] p-2 text-black shadow-lg">
      <span className="text-sm font-bold" style={{ color: card.suit.color }}>
        {card.rank}
      </span>
      <span className="text-center text-3xl" style={{ color: card.suit.color }}>
        {card.suit.mark}
      </span>
      <span className="text-right text-sm font-bold" style={{ color: card.suit.color }}>
        {card.rank}
      </span>
    </div>
  );
...
export function BlackjackTable() {
  ...
  return (
    <div className="rounded-3xl border border-[var(--line)] p-5 felt">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-[var(--gold-2)]">Blackjack</p>
          <h2 className="font-serif text-3xl">Mesa 21</h2>
        </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_220px]">
        <div>
          ...
          <p className="mb-2 mt-6 text-xs uppercase tracking-widest text-white/60">Tú {player.length ? value(player) : ""}</p>
          <div className="flex gap-2">
            {player.map((card, i) => (
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button type="button" onClick={start} className="rounded-xl bg-[var(--gold)] px-4 py-2 font-semibold text-black">
              Repartir
            </button>
            <button type="button" onClick={hit} className="rounded-xl border border-white/20 px-4 py-2">
              Pedir
            </button>
            <button type="button" onClick={stand} className="rounded-xl border border-white/20 px-4 py-2">
              Plantarse
            </button>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full rounded-xl border border-red-400/40 px-3 py-2 text-sm"
          >
          ...
          <button
            ...
            className="w-full rounded-xl border border-white/15 px-3 py-2 text-sm"
          >
            Abrir chat de voz
          </button>
